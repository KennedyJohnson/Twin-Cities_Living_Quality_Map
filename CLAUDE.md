...
- **Home value prediction (Lasso vs. gradient boosting)** (2026-09-22): `pipeline/analysis/home_value_prediction/` — a standalone model-comparison analysis, not part of `build.py`/the scheduled refresh. `fetch_panel.py` reuses `cleaners/clean_housing_price.py` across 2017-2024 (8 years, vs. the frontend's own 2018-2022 affordability trend charts) to build a 224-row district-year panel (`data/panel.csv`, committed). `train.py` (no API key needed — reads the committed panel) predicts next-year median home value from this year's features, selects features via leave-one-year-out CV forward selection, compares `LassoCV` against `GradientBoostingRegressor` on a held-out final-year test transition, and runs two learning-curve sweeps (more training years vs. more features) — writes `results.json`, copied to `web/public/data/home_value_prediction.json`. Write-up page at `web/app/home-value-model/page.tsx` (linked from the main map's nav and from `/about`). Key findings (see the module's `README.md` for the full writeup): gradient boosting never beat Lasso at this sample size (~170 training rows); more training years didn't help either model (2020-2022's pandemic-era price boom appears to be a regime shift older years don't teach), while more features did help. `pipeline/tests/test_home_value_prediction.py` runs against the committed panel in CI (no network/API key needed).
