...
- **"Find Your Match" apartments-vs-houses filter** — implemented 2026-09-23 via Supabase, see "Supabase" below.

#### Supabase (added 2026-09-23)
- Project `wnzawjzruzvxwdmhjxcb`; schema in `supabase/migrations/` (applied manually in the SQL Editor). Keys: `pipeline/.env` (`SUPABASE_URL`, `SUPABASE_SECRET_KEY`, also GitHub secrets), `web/.env.local` + Vercel (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`). All Supabase features no-op if keys are unset.
- **Score history:** `exports/export_supabase.py` (runs after export_all in refresh-data.yml) appends `refresh_runs` + one `area_snapshots` row per district/ZIP. It also runs health checks against the previous run (score jumps >=10, metric counts doubling/halving/zeroing, missing areas) -> `refresh_runs.warnings` + GitHub `::warning::` annotations. Sidebar trend chart: `web/components/ScoreHistoryChart.tsx`.
- **Houses:** `exports/export_houses_supabase.py` syncs ~215K buildings into PostGIS `houses`: ~102K explicitly tagged house/detached/semidetached_house/terrace/bungalow plus ~113K `inferred_house` (untagged `building=yes` imports, 70-400 m^2 footprint, no name/shop/amenity/etc. tags — without these, import-heavy areas like Highland Park, 93% `building=yes`, showed ~5% of their houses); the map fetches per viewport via the `houses_in_bbox()` RPC at zoom >= `HOUSES_MIN_ZOOM` (legend toggle, off by default; clicks reuse the apartment-select flow). Find Your Match (`lib/matchRegions.ts` `housesInRegion`) adds houses matching the House/Townhome/Multi-family checkboxes to each region; apartments still anchor regions since only they carry precomputed radius scores.

#### Cross-City Comparability Fixes (2026-09-22 data audit)
- **St. Paul crime excluded non-crime police activity:** `clean_crime.py` drops `INCIDENT` in {Proactive Police Visit, Community Event}. That was ~53% of rows, and it roughly doubled St. Paul's crime rate vs. Minneapolis. It also made the rate partly a measure of patrol intensity.
- **Minneapolis crime drops non-offense/duplicate categories:** `clean_crime_mpls.py` drops "Subset of NIBRS Assault/Robbery" and "Gunshot Wound Victims" (99.5-100% same-day/same-address duplicates of existing records), plus "Shots Fired Calls" (911/ShotSpotter activations, not confirmed incidents).
- **Permits harmonized:** `clean_permits.py` keeps only the St. Paul `FOLDER_TYPE`s that match Minneapolis's CCS_Permits categories (building, mechanical, plumbing, warm air/ventilation, demolition). It drops electrical (~24% of rows), fence, fire, elevator, sign, and stucco. `clean_permits_mpls.py` dedupes by `permitNumber`, because the feed repeats a permit once per parcel (~19% of rows; 1201 Yale Pl alone was 13 permits x 510 rows).
- **Entertainment added to `sources_mpls.json`:** it was St. Paul-only, so it was normalized among St. Paul districts alone. Minneapolis amenity weights are aligned to 0.2 to match `sources.json`.
- **Net effect:** overall rankings barely moved (Spearman 0.98 vs. before). The city-level component means corrected: Safety SP 50.5->53.4 / MPLS 52.6->47.4. The Opportunity gap narrowed (54.5/43.6 -> 53.4/45.7) and is no longer significant.
- **Remaining, disclosed on the About page:** poverty + diversity explain ~60% of score variance. CDC PLACES chronic disease is model-based (r=-0.85 with bachelor's share), and per-resident rates overstate low-population downtowns. A daytime-population denominator (Census LODES jobs) would be the fix for the latter.
